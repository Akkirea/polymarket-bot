...
class OrderBookFeed:
    """Real-time BTC/USDT order book imbalance from Binance depth stream."""

    _WS_URLS = [
        "wss://stream.binance.com:9443/ws/btcusdt@depth10@100ms",
        "wss://stream.binance.com:443/ws/btcusdt@depth10@100ms",
    ]
    _REST_URL = "https://api.binance.com/api/v3/depth"

    def __init__(self):
        self._imbalance_history: deque = deque(maxlen=600)  # (ts, imbalance), ~10 min
        self._last_update: Optional[float] = None
        self._running = False
        self._ws = None

    @staticmethod
    def _calc_imbalance(bids: list, asks: list) -> Optional[float]:
        try:
    ...
    def get_imbalance(self) -> Optional[float]:
        """Most recent imbalance, or None if data is stale (>5s old)."""
        if not self._last_update or (time.time() - self._last_update) > 5.0:
            return None
        if not self._imbalance_history:
            return None
        return self._imbalance_history[-1][1]

    def get_sustained_imbalance(self, min_seconds: float = 5.0) -> Optional[float]:
        """
        Average imbalance over the last min_seconds, but only if every reading
        in that window agrees on direction. Returns None if mixed or sparse.
        """
        if not self._imbalance_history:
            return None
        cutoff = time.time() - min_seconds
        recent = [(ts, v) for ts, v in self._imbalance_history if ts >= cutoff]
        if len(recent) < 3:
            return None
        avg = sum(v for _, v in recent) / len(recent)
        if all(v > 0 for _, v in recent) or all(v < 0 for _, v in recent):
            return avg
        return None
        ...
        while self._running:
            ...
            try:
                async with websockets.connect(url, ping_interval=20, ping_timeout=10) as ws:
                    ...
                    ws_fail_streak = 0
                    url_index = 0
                    print(f"[ob] WebSocket connected: {url}", flush=True)
                    while self._running:
                        try:
                            msg = await asyncio.wait_for(ws.recv(), timeout=30)
                            data = json.loads(msg)
                            imb = self._calc_imbalance(data.get("bids", []), data.get("asks", []))
                            if imb is not None:
                                now = time.time()
                                self._imbalance_history.append((now, imb))
                ...
                self._ws = None
                ws_fail_streak += 1
                print(f"[ob] WS failed ({type(exc).__name__}), streak={ws_fail_streak}", flush=True)

            except Exception as exc:
                self._ws = None
                ws_fail_streak += 1
                print(f"[ob] WS error ({type(exc).__name__}: {exc}), streak={ws_fail_streak}", flush=True)
    ...
    async def _rest_poll(self, duration: float = 60.0) -> None:
        import aiohttp as _aio
        print(f"[ob] REST fallback active for {duration:.0f}s", flush=True)
        deadline = time.time() + duration
        async with _aio.ClientSession(timeout=_aio.ClientTimeout(total=5)) as session:
            while self._running and time.time() < deadline:
                try:

...
def init_htf_tables():
    """Create HTF-specific tables (idempotent). Called from init_db()."""
    conn = get_connection()
    conn.executescript(f"""
        CREATE TABLE IF NOT EXISTS btc_htf_snapshots (
            id              {_PK},
            market_slug     TEXT NOT NULL,
            btc_spot        REAL NOT NULL,
            market_prob_up  REAL NOT NULL,
            est_prob_up     REAL NOT NULL,
            edge            REAL NOT NULL,
            sigma           REAL,
            recorded_at     TEXT NOT NULL
        );

        CREATE TABLE IF NOT EXISTS btc_htf_trades (
            id               {_PK},
            market_slug      TEXT NOT NULL,
            side             TEXT NOT NULL,
            stake            REAL NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS btc_htf_bot_state (
            id        INTEGER PRIMARY KEY,
            balance   REAL NOT NULL DEFAULT 10000,
        ...
        CREATE INDEX IF NOT EXISTS idx_htf_snapshots_slug ON btc_htf_snapshots(market_slug, recorded_at)
    """)
    conn.commit()

    # Seed bot state row
    conn.execute(
        """INSERT INTO btc_htf_bot_state (id, balance, total_pnl, updated_at)
               VALUES (1, 10000, 0, %s)
           ON CONFLICT(id) DO NOTHING"""
        if _USE_PG else
        """INSERT OR IGNORE INTO btc_htf_bot_state (id, balance, total_pnl, updated_at)
               VALUES (1, 10000, 0, %s)""",
        (datetime.utcnow().isoformat(),),
    )
    conn.commit()
...
def htf_save_snapshot(slug: str, spot: float, mkt_prob: float, est_prob: float,
                       edge: float, sigma: float):
    conn = get_connection()
    conn.execute(
        """INSERT INTO btc_htf_snapshots
               (market_slug, btc_spot, market_prob_up, est_prob_up, edge, sigma, recorded_at)
               VALUES (%s, %s, %s, %s, %s, %s, %s)""",
        (slug, round(spot, 2), round(mkt_prob, 4), round(est_prob, 4),
         round(edge, 4), round(sigma, 4), datetime.utcnow().isoformat()),
    )
    conn.commit()
    conn.close()


def htf_has_open_trade(slug: str) -> bool:
    conn = get_connection()
    row = conn.execute(
        "SELECT id FROM btc_htf_trades WHERE market_slug = %s AND closed_at IS NULL LIMIT 1",
        (slug,),
    ).fetchone()
    conn.close()
...
def htf_open_trade(slug: str, side: str, stake: float, entry_prob: float,
                    entry_spot: float, edge: float):
    conn = get_connection()
    conn.execute(
        """INSERT INTO btc_htf_trades
               (market_slug, side, stake, entry_prob, entry_spot, entry_edge, opened_at)
               VALUES (%s, %s, %s, %s, %s, %s, %s)""",
        (slug, side, round(stake, 2), round(entry_prob, 4), round(entry_spot, 2),
         round(edge, 4), datetime.utcnow().isoformat()),
    )
    conn.commit()
    conn.close()


def htf_close_trades(slug: str, winner: str, final_price, price_to_beat):
    """Resolve all open HTF trades for a slug. winner is 'up' or 'down'."""
    conn = get_connection()
    rows = conn.execute(
        """SELECT id, side, stake, entry_prob
             FROM btc_htf_trades
            WHERE market_slug = %s AND closed_at IS NULL""",
        (slug,),
    ).fetchall()

    now = datetime.utcnow().isoformat()
    for row in rows:
        won = row["side"].lower() == winner.lower()
        ep  = float(row["entry_prob"])
        stake = float(row["stake"])
        pnl = round((stake / ep - stake) if won else -stake, 2)
        conn.execute(
            """UPDATE btc_htf_trades
                  SET closed_at = %s, outcome = %s, pnl = %s,
                      resolution_price = %s, price_to_beat = %s
                WHERE id = %s""",
            (now, "WIN" if won else "LOSS", pnl,
        ...
        # Update bot state
        conn.execute(
            "UPDATE btc_htf_bot_state SET balance = balance + %s, "
            "total_pnl = total_pnl + %s, updated_at = %s WHERE id = 1",
            (pnl, pnl, now),
        )
...
def htf_get_state() -> dict:
    conn = get_connection()
    row = conn.execute(
        "SELECT balance, total_pnl, updated_at FROM btc_htf_bot_state WHERE id = 1"
    ).fetchone()
    conn.close()
    if row:
        return {"balance": float(row["balance"]), "total_pnl": float(row["total_pnl"]),
                "updated_at": row["updated_at"]}
    return {"balance": 10000.0, "total_pnl": 0.0, "updated_at": ""}


def htf_get_recent_trades(n: int = 20) -> list:
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM btc_htf_trades ORDER BY opened_at DESC LIMIT %s", (n,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def htf_get_latest_snapshots() -> list:
    """Return the most recent snapshot per market."""
    conn = get_connection()
    if _USE_PG:
        rows = conn.execute(
            """SELECT DISTINCT ON (market_slug)
                      market_slug, btc_spot, market_prob_up, est_prob_up, edge, sigma, recorded_at
                 FROM btc_htf_snapshots
                ORDER BY market_slug, recorded_at DESC"""
        ).fetchall()
    else:
        rows = conn.execute(
            """SELECT market_slug, btc_spot, market_prob_up, est_prob_up, edge, sigma,
...
def htf_get_history(slug: str, hours: int = 6) -> list:
    conn = get_connection()
    if _USE_PG:
        rows = conn.execute(
            """SELECT market_slug, btc_spot, market_prob_up, est_prob_up, edge, recorded_at
                 FROM btc_htf_snapshots
                WHERE market_slug = %s
        ...
        cutoff = datetime.utcnow().isoformat()[:13]  # rough hour cutoff
        rows = conn.execute(
            """SELECT market_slug, btc_spot, market_prob_up, est_prob_up, edge, recorded_at
                 FROM btc_htf_snapshots
                WHERE market_slug = %s
